...
CREATE TABLE refugios (
                          ...
                          ref_lat               DECIMAL(9,6) NULL,
                          ref_lng               DECIMAL(9,6) NULL,
                          ref_activa            BOOLEAN DEFAULT TRUE,
                          ref_estado_aprobacion VARCHAR(20) DEFAULT 'APROBADA'
);
...
CREATE TABLE veterinarias (
                              ...
                              vet_foto              VARCHAR(255),
                              vet_lat               DECIMAL(9,6) NULL,
                              vet_lng               DECIMAL(9,6) NULL,
                              vet_estado_aprobacion VARCHAR(20) DEFAULT 'APROBADA'
);

CREATE TABLE alertas (
                         alt_id          BIGINT AUTO_INCREMENT PRIMARY KEY,
                         alt_mascota_id  BIGINT,
                         alt_usuario_id  BIGINT NOT NULL,
...
CREATE TABLE reportes (
                          ...
                          rep_refugio_id     BIGINT,
                          rep_alerta_id      BIGINT,
...
CREATE TABLE adopciones (
                            ado_id                BIGINT AUTO_INCREMENT PRIMARY KEY,
                            ado_mascota_id        BIGINT NOT NULL,
                            ado_usuario_id        BIGINT NOT NULL,
                            ...
                            ado_foto              VARCHAR(255),
                            ado_ubicacion         VARCHAR(255),
                            ado_refugio_id        BIGINT NULL,
                            FOREIGN KEY (ado_mascota_id) REFERENCES mascotas(msc_id),
                            FOREIGN KEY (ado_usuario_id) REFERENCES usuarios(usr_id),
                            FOREIGN KEY (ado_refugio_id) REFERENCES refugios(ref_id)
);

CREATE TABLE postulaciones (
                               pos_id          BIGINT AUTO_INCREMENT PRIMARY KEY,
                               pos_adopcion_id BIGINT NOT NULL,
                               pos_usuario_id  BIGINT NOT NULL,
...
CREATE INDEX idx_alertas_lat_lng  ON alertas  (alt_lat, alt_lng);
CREATE INDEX idx_reportes_lat_lng ON reportes (rep_lat, rep_lng);
